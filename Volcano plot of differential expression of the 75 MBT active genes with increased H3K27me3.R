...
rm(list=ls())
dataset <- read.csv("75gene_pva.csv",header = T)
...
p <- ggplot(
  ...
  scale_color_manual(values=c("#546de5", "#2A2C2D","#DA070E"))+
  xlim(c(-1.5,1.5))+ ylim(c(-0.0002,6)) + 
...
p + geom_text_repel(data = dataset, aes(x = log2FoldChange, 
                    ...
                    segment.color = "black", 
                    show.legend = FALSE)
ggsave("E://DEG_volcano.pdf",width=7,height=4) 
